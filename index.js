...
// both functions are under the same scope so we can pass variables down but we can't pass them back up
...
function summation(mamboNumberFive) {
  let sum = 0;
  for (let i = 1; i <= mamboNumberFive; i++) {
    sum = sum + i;
  }
  return sum;
}
...
function lowerCaseNames(animals) {
  const lowerNames = animals.map(function (animal) {
...
function lowPopulationAnimals(animals) {
  const lowPopAnimals = animals.filter(function (animal) {
...
function USApop(animals) {
  return animals.reduce(function (previousvalue, currentvalue) {
...
consume("a", "b", function (a, b) {
  console.log("argumento1:", a, "argumento2", b);
});
...
const cuboidTwo = new CuboidMakerTwo({ length: 4, height: 5, width: 5 });
//🦄🦄🦄 Test your volume and surfaceArea methods by uncommenting the logs below: 🦄🦄🦄
